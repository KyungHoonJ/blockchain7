...
class Wallet {
  ...
  static verify(_receivedTx: {
    ...
    if (global.debug) console.log("5-11/6-13/8-11 서명 확인");
  ...
  static sendTransaction(
    ...
    if (global.debug) console.log("6-12/8-10 트랜잭션 추가 함수 실행");
    const isValid = Wallet.verify(_receivedTx);
    if (global.debug) console.log("6-14/8-12 서명 문제 있으면 끝");
    if (isValid.isError === true) return isValid;

    if (global.debug) console.log("6-15/8-13 지갑 객체 생성");
    const wallet = new this(_receivedTx.sender, _receivedTx.signature, _utxos);
    if (global.debug) console.log("6-19/8-14 잔액과 보낼 금액 확인");
    if (wallet.balance < _receivedTx.amount) {
      return { isError: true, msg: "잔액 부족" };
    }

    if (global.debug) console.log("6-20/8-15 보내는 사람의 utxo 목록 가져오기");
    const myUTXO = UnspentTxOut.getMyUTXO(wallet.address, _utxos);
    if (global.debug) console.log("6-22/8-17 트랜잭션 생성 함수 호출");
